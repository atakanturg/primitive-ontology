...
${newsData.status === "fulfilled" ? newsData.value : "Unavailable"}`.trim();
    ...
    const systemPrompt = `You are an elite quantitative analyst with deep expertise in equity research... [rest of your system prompt]`;

    const userPrompt = `Analyze ticker $${ticker}. ${experimentalInstruction}\n\n--- DATA BUNDLE ---\n${bundle}`;

    // FIX: Using getGenerativeModel correctly
...
async function generateSearchKeywords(ticker: string, ai: any) {
  ...
  const result = await model.generateContent({
    contents: [{ 
      role: "user", 
      parts: [{ text: `Given ticker "${ticker}", return JSON with: "companyName", "researchKeywords", "newsKeywords".` }] 
    }],
    generationConfig: { responseMimeType: "application/json" },
  });
